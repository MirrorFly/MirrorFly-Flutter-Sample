...
class SessionManagement {
  ...
  static Future setPIN(String pin) async {
    ...
    await _preferences.setInt(
        Constants.changedPinAt, DateTime.now().millisecondsSinceEpoch);
  }
  ...
  static Future setSummaryChannelId(String summaryChannelId) async {
    await _preferences.setString(
        "$prefix${Constants.packageName}summary_channel.id", summaryChannelId);
  }
  ...
  static Future setNotificationVibration(bool vibration) async {
    await _preferences.setBool(
        "$prefix${Constants.package}vibration", vibration);
  }
  ...
  static void setLockExpiry(int expiryTimeStamp) async {
    await _preferences.setInt(
        "$prefix${Constants.expiryDate}", expiryTimeStamp);
  }
  ...
  static Future setUser(Data data) async {
    ...
    await _preferences.setString(
        '${prefix}username', data.username.checkNull());
    await _preferences.setString(
        '${prefix}password', data.password.checkNull());
