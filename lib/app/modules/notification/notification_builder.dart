...
class NotificationBuilder {
  ...
  static createNotification(ChatMessage message) async {
    ...
    var messagingStyle =
        MessagingStyleInformation(const Person(name: "Me"), messages: []);
    ...
    if (notificationModel != null) {
      ...
      if (Platform.isIOS) {
        ...
        if (isMessageRecalled) {
          // if message was recalled then rebuild the message style
        ...
        debugPrint("inside lastMessageContent $lastMessageContent");
  ...
  static appendChatMessageInMessageStyle(StringBuffer messageContent,
    ...
    debugPrint("received messageContent $messageContent");
    ...
    debugPrint("received messageContent after $messageContent");
    ...
    if (Platform.isAndroid) {
      ...
      if (userProfileImage != null) {
        ...
        messagingStyle.messages?.add(Message(
            ...
            DateTime.fromMillisecondsSinceEpoch(message.messageSentTime),
            Person(name: name, icon: image)));
  ...
  static Future<Uint8List?> getUserProfileImage(Profile profileDetails) async {
    ...
    LogMessage.d(
        "profileDetails.isGroupProfile", profileDetails.isGroupProfile);
    if (imgUrl.isNotEmpty) {
      // return const FlutterBitmapAssetAndroidIcon(profileImg);
      return await downloadAndShow(
          SessionManagement.getMediaEndPoint().checkNull() + imgUrl,
          profileDetails.name.checkNull());
    } else {
  ...
  static MessagingStyleInformation setConversationTitleMessagingStyle(
    ...
    if (messageCount > 1) {
      ...
      if (profileDetails.isGroupProfile ?? false) {
        ...
        var newMessageStyle = MessagingStyleInformation(messagingStyle.person,
  ...
  static displaySummaryNotification(StringBuffer lastMessageContent) async {
    ...
    summaryText.write(
        "${getMessagesCount()} messages from ${chatNotifications.length} chats");
    var inlineMessages = <String>[];
    chatNotifications.forEach((key, value) {
      inlineMessages.add("notificationInlineMessage");
    });
    var inboxStyle = InboxStyleInformation(inlineMessages,
        summaryText: summaryText.toString(), contentTitle: appTitle);

    var channel = buildNotificationChannel(
        Constants.emptyString, summaryChannelName, true);
    ...
    if (chatNotifications.isNotEmpty) {
      ...
      await flutterLocalNotificationsPlugin
          .resolvePlatformSpecificImplementation<
              AndroidFlutterLocalNotificationsPlugin>()
          ?.createNotificationChannel(channel);
      await flutterLocalNotificationsPlugin.show(summaryId, appTitle,
          lastMessageContent.toString(), notificationDetails,
          payload: "");
    }
  }

  static Future<AndroidBitmap<Object>?> getLargeUserOrGroupImage(
  ...
  static Future<Uint8List?> downloadAndShow(String url, String fileName) async {
    ...
    final http.Response response = await http.get(Uri.parse(url), headers: {
      "Authorization": SessionManagement.getAuthToken().checkNull()
    });
