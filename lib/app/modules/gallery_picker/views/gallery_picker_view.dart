...
class GalleryPickerView extends NavViewStateful<GalleryPickerController> {
  ...
  @override
  GalleryPickerController createController({String? tag}) =>
      Get.put(GalleryPickerController());

  @override
  Widget build(BuildContext context) {
    return Theme(
      data: Theme.of(context)
          .copyWith(appBarTheme: AppStyleConfig.galleryPageStyle.appBarTheme),
      child: Scaffold(
        appBar: AppBar(
          title: Text(getTranslated("sendToUser")
              .replaceAll("%d", "${controller.userName}")),
        ),
        body: Column(
          children: [
            Expanded(
                              ...
                              GestureDetector(
                                onTap: () async {
                                  ...
                                  if (controller.pickedFile.isNotEmpty) {
                                    ...
                                    //
                                    NavUtils.toNamed(Routes.mediaPreview,
                                        arguments: {
                                          ...
                                          "captionMessage":
                                              controller.captionMessage,
                                          "captionMessageMentions":
                                              controller.captionMessageMentions,
                                        })?.then((value) {
                                      if (value == null &&
                                          NavUtils.currentRoute !=
                                              Routes.chat) {
                                        NavUtils.back();
                                      } else {
                                        controller.captionMessage =
