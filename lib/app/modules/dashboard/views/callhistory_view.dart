...
class CallHistoryView extends StatelessWidget {
  const CallHistoryView(
      ...
      this.recentCallsTitleStyle = const TextStyle(
        ...
        Obx(() {
          return controller.callLogList.isEmpty
              ...
              : Container(
                  ...
                  child: Text(
                    getTranslated("recentCalls"),
                    style: recentCallsTitleStyle,
                  ));
        }),
        Expanded(
          child: Stack(
        ...
        itemBuilder: (context, index) {
          ...
          return item.callMode == CallMode.oneToOne &&
              ...
              ? Obx(() => ListTile(
                    ...
                    shape: Border(
                        bottom: BorderSide(
                            color: callHistoryItemStyle.dividerColor,
                            width: 0.5)),
                    leading: FutureBuilder(
                        future: getProfileDetails(item.callState == 1
                            ? item.toUser!
                            : item.fromUser!),
                        builder: (context, snap) {
                          return snap.hasData && snap.data != null
                              ? ImageNetwork(
                                  ...
                                  clipOval: true,
                                  errorWidget: snap.data!
                                          .getName() //item.nickName
                                          .checkNull()
                                          .isNotEmpty
                                      ? ProfileTextImage(
                                          text: getName(snap.data!),
                                          radius: callHistoryItemStyle
                                                  .profileImageSize.height /
                                              2,
                                        )
                                      : const Icon(
                    ...
                    subtitle: SizedBox(
                      child: callLogTime(
                          "${DateTimeUtils.getCallLogDate(microSeconds: item.callTime!)}  ${getChatTime(context, item.callTime)}",
                          item.callState,
                    ...
                    leading: item.groupId.checkNull().isEmpty
                        ? ClipOval(
                            child: AppUtils.assetIcon(
                              ...
                              height:
                                  callHistoryItemStyle.profileImageSize.height,
                              fit: BoxFit.cover,
                            ),
                          )
                        : FutureBuilder(
                    ...
                    trailing: SizedBox(
                      child: Row(
                        ...
                        children: [
                          ...
                          callIcon(item.callType, item, item.callMode,
                              item.userList, callHistoryItemStyle.iconColor),
                        ],
                      ),
                    ),
                    selectedTileColor: controller.isLogSelected(index)
    ...
    return callType!.toLowerCase() == CallType.video
        ? IconButton(
            onPressed: () {
              callMode == CallMode.oneToOne
                  ? controller
                      .makeVideoCall(item.callState == CallState.missedCall
                          ? item.fromUser
                          : item.callState == CallState.incomingCall
                              ? item.fromUser
                              : item.toUser)
        ...
        : IconButton(
            onPressed: () {
              callMode == CallMode.oneToOne
                  ? controller
                      .makeVoiceCall(item.callState == CallState.missedCall
                          ? item.fromUser
                          : item.callState == CallState.incomingCall
                              ? item.fromUser
                              : item.toUser)
