...
class ProfileController extends GetxController {
    ...
    if(profileName.text.isEmpty){
     ...
     toToast("Enter Profile Status");
      ...
      PlatformRepo().updateProfile(profileName.text.toString(),
          ...
          profileStatus.value.toString(),userImgUrl.value.isEmpty ? null : userImgUrl.value).then((value) {
            loading.value=false;
            var data = profileUpdateFromJson(value);
            if(data.status!=null) {
              toToast(data.message.toString());
              Get.arguments!=null ? Get.toNamed(Routes.DASHBOARD) : Get.back();
            }
      }).catchError((error) {
        loading.value=false;
  ...
  updateProfileImage(String imagepath){
    loading.value=true;
    PlatformRepo().updateProfileImage(imagepath).then((value){
      loading.value=false;
    ...
    PlatformRepo().removeProfileImage().then((value) {
      ...
      if (value != null) {
        if (value) {
          ...
          userImgUrl.value = Constants.EMPTY_STRING;
  ...
  getProfile(){
    loading.value=true;
    PlatformRepo().getProfile(SessionManagement().getUserJID()!).then((value){
      loading.value=false;
  ...
  Future ImagePicker(BuildContext context) async {
    ...
    if (result != null) {
      ...
      Get.to(CropImage(imageFile: File(imagepath.value),))?.then((value){
