...
/// {@macro builder}
typedef ChatTaggerWidgetBuilder = Widget Function(
...
typedef ChatTaggerSearchCallback = void Function(
  String query,
  String triggerCharacter,
);

///{@macro showOrHideTagsCallback}
typedef ShowOrHideChatTaggerCallback = void Function(
  bool show,
);
...
///
/// [ChatTagger] maintains tag positions during text editing and allows
...
class ChatTagger extends StatefulWidget {
  /// Creates a ChatTagger widget.
  ///
  ...
  const ChatTagger({
    ...
    required this.onShowOrHideTaggers,
  ...
  /// {@macro ChatTaggerController}
  final ChatTaggerController controller;
  ...
  /// whenever [ChatTagger] enters the search context.
  /// {@endtemplate}
  final ChatTaggerSearchCallback onSearch;

  /// {@template showOrHideTagsCallback}
  /// Called with the show or hide tagger list
  /// whenever its need to show or hide
  /// {@endtemplate}
  final ShowOrHideChatTaggerCallback onShowOrHideTaggers;

  /// {@template builder}
  /// Widget builder for [ChatTagger]'s associated TextField.
  ///  {@endtemplate}
  /// Returned widget should be the [TextField] with the [GlobalKey] as its key.
  final ChatTaggerWidgetBuilder builder;
  ...
  @override
  State<ChatTagger> createState() => _ChatTaggerState();
}

class _ChatTaggerState extends State<ChatTagger> {
  ChatTaggerController get controller => widget.controller;

  late final _textFieldKey = GlobalKey(
    debugLabel: "ChatTagger's child TextField key",
  );
  ...
  void _shouldHideOverlay(bool val) {
    widget.onShowOrHideTaggers(!val);
    try {
  ...
  /// of calling [ChatTagger.tagTextFormatter] if it's not null.
  ...
  Map<int,Map<bool,TaggedText?>> tagIndex(int index){
    var defaultValue = <int,Map<bool,TaggedText?>>{index: {false:null}};
    var it  = _tags.keys.toList();
    if(it.isEmpty){
      return defaultValue;
    }
    for (int i = 0; i < it.length; i++) {
      if(it[i].startIndex <= index&& index <= it[i].endIndex){
        return <int,Map<bool,TaggedText?>>{index: {true:it[i]}};
      }
    }
    return defaultValue;//const Tuple2(false,null);
  }
...
/// {@template ChatTaggerController}
/// Controller for [ChatTagger].
/// This object exposes callback registration bindings to enable clearing
/// [ChatTagger]'s tags, dismissing overlay and retrieving formatted text.
/// {@endtemplate}
class ChatTaggerController extends TextEditingController {
  ChatTaggerController({String? text}) : super(text: text);
  ...
  /// Formatted text from [ChatTagger]
  String get formattedText => _text;

  Function? _formatTagsCallback;

  /// {@template formatTags}
  /// Extracts tags from [ChatTaggerController]'s [text] and formats the textfield to display them as tags.
  /// This should be called after [ChatTaggerController] is constructed with a non-null
  /// text value that contain unformatted tags.
  ///
  /// [pattern] -> Pattern to match tags.
  /// Specify this if you supply your own [ChatTagger.tagTextFormatter].
  ///
  ...
  /// Clears [ChatTagger] internal tag state.
  @override
  void clear() {
    _clearCallback?.call();
    _text = '';
  ...
  /// Registers callback for clearing [ChatTagger]'s
  /// internal tags state.
  void _onClear(Function callback) {
    _clearCallback = callback;
  }

  /// Registers callback for dismissing [ChatTagger]'s overlay.
  void _onDismissOverlay(Function callback) {
    _dismissOverlayCallback = callback;
  }

  /// Updates [_text] with updated and formatted text from [ChatTagger].
  ...
  TextSpan buildTextSpan({
    ...
    // return findTags(style);
    var buildTextSpan =  _buildTextSpan(style);
    /*var buildTextSpanOld =  _buildTextSpanOld(style);
    LogMessage.d("_buildTextSpan","buildTextSpanOld : ${buildTextSpanOld.children}");
    LogMessage.d("_uildTextSpan","buildTextSpanNew : ${buildTextSpan.children}");*/
  ...
  /*[
  TextSpan(""),
  TextSpan(inherit: true, color: MaterialColor(primary value: Color(0xfff44336)), "@Flutter"),
  TextSpan(" "),
  TextSpan("hi "),
  TextSpan(""),
  TextSpan(inherit: true, color: MaterialColor(primary value: Color(0xfff44336)), "@flutter 7"),
  TextSpan(" "),
  TextSpan(" ")]*/
  ...
  /*TextSpan _buildTextSpanOld(TextStyle? style) {
    if (text.isEmpty) return const TextSpan();
    ...
    for (int i = 0; i < splitText.length; i++) {
      LogMessage.d("_buildTextSpan","skipIndex : $skipIndex , end : $end",);
      if(skipIndex!=null && skipIndex == end){
        skipIndex=null;
        continue;
      }
      ...
      if (currentText.contains(_triggerCharactersPattern)) {
        ...
        final taggedSpans = findTags(start);
        LogMessage.d("_buildTextSpan","nestedSpans : $nestedSpans",);
        LogMessage.d("_buildTextSpan","taggedSpans : $taggedSpans");
        LogMessage.d("_buildTextSpan","start : ${taggedSpans.item2}");
        spans.addAll(taggedSpans.item1);
        skipIndex = taggedSpans.item2;
  ...
  Tuple2<List<TextSpan>,int?> findTags(int start){
    // log("${_tags.keys}",name: "_buildTextSpan");
    // log("${_tags.values}",name: "_buildTextSpan value");
    List<TextSpan> spans = [];
    var it = _tags.keys.iterator;
    int? endIndex;
    while(it.moveNext()){
      var current = it.current;
      var currentText = text.substring(current.startIndex,current.endIndex);
      var triggerChar = text.substring(current.startIndex,current.startIndex+1);
      // log("triggerChar : $triggerChar",name: "_buildTextSpan");
      // log("currentText : $currentText",name: "_buildTextSpan");
      if(start==current.startIndex) {
        spans.add(const TextSpan(text: ""));
        spans.add(
          TextSpan(
            text: currentText,
  ...
  Tuple2<List<TextSpan>,int?> findTag(){
    // log("${_tags.keys}",name: "_buildTextSpan");
    // log("${_tags.values}",name: "_buildTextSpan value");
    List<TextSpan> spans = [];
    var it = _tags.keys.iterator;
